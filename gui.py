...
class RestoreTool(Frame):
    def __init__(self, master, frame):
        ...
        self.button = ttk.Button(middle_frame, text="Select Image", command=self.load_image).grid(row=0, column=1,
                                                                                                  pady=(padding_top, 0),
                                                                                                  sticky='ns')

        self.label_image_path = ttk.Label(middle_frame, text="No Image selected", anchor='center', justify='center')
        self.label_image_path.grid(row=1, column=1, pady=(int(frame_height * 0.01), 0),
                                   padx=(middle_frame_width * 0.1, middle_frame_width * 0.1), sticky='ns')
        self.label_image_path.config(wraplength=self.calculate_wraplength(middle_frame), anchor='center',
                                     justify='center')

        # Drop-Down Menu Options
        options = ["-- select option --", "Custom RGBA", "Custom RGB", "Windows 11 Snipping Tool", "Google Pixel 3",
                   "Google Pixel 3 XL", "Google Pixel 3a", "Google Pixel 3a XL", "Google Pixel 4", "Google Pixel 4 XL",
                   "Google Pixel 4a", "Google Pixel 5", "Google Pixel 5a", "Google Pixel 6", "Google Pixel 6 Pro",
        ...
        self.width_label = ttk.Label(middle_frame, text="Original Width:").grid(row=3, column=1,
                                                                                pady=int(frame_height * 0.003))
        self.width_entry = ttk.Entry(middle_frame)
        self.width_entry.grid(row=4, column=1, pady=int(frame_height * 0.01),
                              sticky='ns')  # Add this line to place the Entry Box in the grid layout
        self.width_entry.configure(justify='center')  # Add this line to center the text in the Entry

        self.height_label = ttk.Label(middle_frame, text="Original Height:").grid(row=5, column=1,
                                                                                  pady=int(frame_height * 0.003))
        ...
        self.button_acrop = ttk.Button(middle_frame, text="Acropalypse Now!", command=self.on_button,
                                       style='my.TButton').grid(row=7, column=1, pady=int(frame_height * 0.01),
                                                                sticky='ns')
    ...
    def load_image(self):
        ...
        if self.cropped_image_file:
            ...
            self.label_log.config(text="Select picture mode and press button to reconstruct", anchor='center',
                                  justify='center')
    ...
    def acrop_now(self):
        if pathlib.Path(self.cropped_image_file).suffix == ".gif":
            try:
                acropalypse_gif.restore(self.cropped_image_file, os.path.join(tempdir, 'restored.gif'),
                                        int(self.width_entry.get() or 1920), int(self.height_entry.get() or 1080))
            ...
            try:
                ...
                self.acropalypse_Instance.reconstruct_image(self.cropped_image_file,
                                                            int(self.width_entry.get() or 1920),
                                                            int(self.height_entry.get() or 1080), self.rgb_alpha)
            except Exception as e:
                self.label_log.config(text=f"Error reconstructing the image: {e}", anchor='center', justify='center')
                self.reconstructing = False
                return
        try:
            ...
            self.label_log.config(
                text=f"Error reconstructing the image! \nAre you using the right mode and resolution?", anchor='center',
                justify='center')
            self.reconstructing = False
    ...
    def save_image(self):
        ...
        if self.right_label.image:
            ...
            file_path = asksaveasfilename(defaultextension=".png",
                                          filetypes=[("PNG Files", "*.png"), ("GIF Files", "*.gif")], parent=self)
            ...
            self.label_log.config(text="No image to save, reconstruct an image first.", anchor='center',
                                  justify='center')
